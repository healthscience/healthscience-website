import { AndesOrganon } from './organs/manifests/andes.manifest.js';
import './organs/organon-needs.js';

export class HsOrganonAndes extends HTMLElement {
    constructor() {
        super();
    ...
    onActivate(subState) {
        // Triggered when peers navigate directly to #organon/andes
        console.log("Andes Organon active state:", subState);
    ...
    injectOrganonData() {
        const needsElement = this.shadowRoot.getElementById('andes-needs-section');
        if (needsElement && typeof needsElement.setOrganonData === 'function') {
            needsElement.setOrganonData(AndesOrganon);
        } else {
            customElements.whenDefined('organon-needs').then(() => {
                const retryElement = this.shadowRoot.getElementById('andes-needs-section');
                if (retryElement) {
                    retryElement.setOrganonData(AndesOrganon);
                }
            });
        }
    }
    ...
    render() {
            ...
            <header class="watershed-header">
                ...
                <div class="header-cta-bar">
                    <button id="jump-needs-btn" class="cta-btn-amber" type="button">
                        <span>lo que hay que hacer: What Needs Done</span> &darr;
                    </button>
                </div>
            </header>
        </div>
        ...
        <organon-needs id="andes-needs-section"></organon-needs>
        `;
    }
}

customElements.define('hs-organon-andes', HsOrganonAndes);