import { BeemsOrganon } from './organs/manifests/beems-na.manifest.js';
import './organs/organon-needs.js';

export class HsOrganonBeems extends HTMLElement {
    constructor() {
        super();
    ...
    onActivate(subState) {
        // Triggered when peers navigate directly to #organon/beems
        console.log("Beems Organon active state:", subState);
    ...
    injectOrganonData() {
        const needsElement = this.shadowRoot.getElementById('beems-needs-section');
        if (needsElement && typeof needsElement.setOrganonData === 'function') {
            needsElement.setOrganonData(BeemsOrganon);
        } else {
            customElements.whenDefined('organon-needs').then(() => {
                const retryElement = this.shadowRoot.getElementById('beems-needs-section');
                if (retryElement) {
                    retryElement.setOrganonData(BeemsOrganon);
                }
            });
        }
    }
    ...
    render() {
        ...
        <organon-needs id="beems-needs-section"></organon-needs>
        `;
    }
}

customElements.define('hs-organon-beems', HsOrganonBeems);